...
public class StreamGroup {
	...
	public static void main(String[] args) throws IOException {
		...
		Map<Boolean, List<Locale>> englishAndOtherLocales = locales
				.collect(Collectors.partitioningBy(l -> l.getLanguage().equals(
						"en")));
		System.out.println("English locales: "
				+ englishAndOtherLocales.get(true));

		locales = Stream.of(Locale.getAvailableLocales()).filter(
				s -> s.getDisplayCountry().length() > 0);
		...
		Stream<City> cities = readCities("./src/ch2/cities.txt");
		...
		cities = readCities("./src/ch2/cities.txt");
		...
		Map<String, Set<String>> countryToLanguages = locales
				.collect(groupingBy(l -> l.getDisplayCountry(),
