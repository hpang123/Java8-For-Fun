...
public class StreamsOfLines {
	public static void main(String[] args) throws IOException {
		Path path = Paths.get("./src/ch8/StreamsOfLines.java");
		try (Stream<String> lines = Files.lines(path)) {
			Optional<String> passwordEntry = lines.filter(
					s -> s.contains("password")).findFirst();
		...
		try (BufferedReader reader = new BufferedReader(new Reader() {
			...
			public void close() {
				System.out.println("Reader is close");
			}
