	...
	public static double run(int nthreads, int iterations, Runnable action) {
		...
		for (int t = 0; t < nthreads; t++) {
			threads[t] = new Thread(() ->
				{
					//await call nthreads times, and wait for the one more  await call
					await(barrier);
		...
		//await will call nthreads +1 times, so it start run next
		await(barrier);
		long start = System.nanoTime();
		//after nthreads finish call nthreads await, one more await call will release await
		await(barrier);
